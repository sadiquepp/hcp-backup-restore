...
| Want the phase-3 client without four VMs | `--tags clabnsclient` builds one VM with one network namespace per tenant; both test scripts take `--netns`. A namespace is stronger client-side isolation than a VRF - no visibility of another namespace's interfaces or routes, and no shared rule table to mis-order - so the objection to one multi-tenant host does not apply. It also drops the grouping rules: blue and red need separate *hosts* only because one routing table holds one route to `10.200.0.0/16`. Built alongside `clabtenantclients`, not instead of it; they coexist because the namespaces take `.21` on each client segment and the VMs `.20`, which `nsclient-vm.yml` asserts |
