...
| Session up, prefixes exchanged, pod ping still fails - and `tcpdump` shows the reply arriving on the fabric NIC but never on `ovn-k8s-mp0` | IP forwarding is off on the fabric NIC. OVN-Kubernetes enables it per interface (`br-ex`, `ovn-k8s-mpN`) and leaves `conf.all.forwarding` at 0, so a NIC it does not manage inherits 0. `ip route get <pod ip> from <leaf ip> iif <nic>` answering "No route to host" is the tell - that is EHOSTUNREACH, which the kernel returns only when forwarding is off, never for a missing route. The fabric NNCP sets `ipv4.forwarding: true` (nmstate 2.2.51+) |
